 ...
 */
import { Fragment, lazy, Suspense, useEffect, useRef, useState, type MouseEvent } from "react";
import {
  BadgeCheck,
  BookOpen,
  Building2,
...
const CourseFaqList = lazy(() => import("./CourseFaqList"));
...
export default function CourseLanding({ campaign }: { campaign: CourseCampaignConfig }) {
  const { city, state, date, schedule, venue, pricing, whatsapp } = campaign;
  const eventDate = `${date.dayOfWeek} ${date.label}`;
  const getWhatsAppHref = (message: string) => `https://wa.me/${whatsapp.number}?text=${encodeURIComponent(message)}`;
  const { presale: presaleWhatsAppMessage, regular: regularWhatsAppMessage, team: teamWhatsAppMessage } = whatsapp.messages;
    ...
    <main className="landing-shell">
      ...
      <section className="hero" id="inicio" aria-labelledby="course-title">
        <div className="hero-copy">
          ...
          <div className="merida-event-details" aria-label={`Detalles del curso presencial en ${city}`}>
            <span><MapPin aria-hidden="true" />{` ${city}, ${state}`}</span>
            <span><CalendarDays aria-hidden="true" />{` ${eventDate}`}</span>
          </div>
        ...
        <div className="pricing-inner">
          ...
          <div className="pricing-grid">
            ...
            <article className="pricing-card pricing-card-regular" aria-labelledby="regular-title">
              <div className="pricing-card-copy">
                ...
                <p className="pricing-current-price">{pricing.regular}</p>
          ...
          <aside className="team-pricing-block" aria-labelledby="team-pricing-title">
            ...
            <div className="team-price-options" aria-label="Precios para parejas y equipos">
              {pricing.teams.map((team) => <div className="team-price-option" key={team.people}><strong>{`${team.people} personas — ${team.total}`}</strong><span>{`${team.approximate ? "Aproximadamente " : ""}${team.perPerson} por persona`}</span></div>)}
            </div>
            ...
            <article className="venue-card">
              ...
              <dl className="venue-details">
                ...
                <div>
                  ...
                  <dd>{venue.addressLines.map((line, index) => <Fragment key={index}>{index > 0 && <br />}{line}</Fragment>)}</dd>
                </div>
              </dl>
      ...
      <section ref={faqSectionRef} className="faq-section" aria-labelledby="faq-title">
        <div className="faq-inner">
          ...
          {shouldLoadFaq && <Suspense fallback={null}><CourseFaqList campaign={campaign} /></Suspense>}
        </div>
      </section>

      <section className="final-cta-section" aria-labelledby="final-cta-title">
        <div className="final-cta-inner">
          <h2 id="final-cta-title" aria-label={`Aprende a aplicar resina epóxica y asegura tu lugar en ${city}`}>
            <span className="final-headline-desktop" aria-hidden="true">
              ...
              <span className="final-headline-line"><strong>{city}</strong>{` este ${date.label}`}</span>
            </span>
            <span className="final-headline-mobile" aria-hidden="true">
              Aprende a aplicar <em>resina epóxica</em> y asegura tu lugar en <strong>{city}</strong>{` este ${date.label}`}
            </span>
          </h2>
          <span className="dark-title-rule" aria-hidden="true" />
          ...
          <p className="final-cta-trust">{`Cupo presencial limitado · Taller práctico en ${venue.label}`}</p>
        </div>
      </section>
